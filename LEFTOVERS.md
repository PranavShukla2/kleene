...
**Phase 5 is mostly closed.** Twenty examples that are also CI fixtures, an installable app
...
| #      | What                                                               | Why it is not done                                                                                                                                                                                                                                                                                                                     | Where it goes                   |
| ------ | ------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- |
| **T1** | No desktop build (Track B)                                         | The largest single thing between here and v1, and not blocked on code — the web build it wraps has been ready since Track A closed. It needs a machine per platform and **D15**, the signing decision. **B4's ~6 MB claim should be measured before B1**, because that figure is already published in the JFLAP comparison.            | Before v1                       |
| **T2** | Algorithm pages are not generated from `Traced` (D3)               | The item worth coming back for. This is where "three consumers of one source of truth" stops being an architecture diagram and becomes something a reader can check. It needs a rendering path from `Step` to prose outside React — a build step nobody has designed.                                                                  | Phase 6                         |
...
| **T4** | No install prompt (A4)                                             | Deliberately last. `beforeinstallprompt` fires when the browser already thinks the app is installable, and a prompt shown before anyone has drawn anything is the interruption every PWA is disliked for. Needs a trigger tied to having done something.                                                                               | Phase 6                         |
